...
1. **High: custom Rust regression forecasts can omit process uncertainty.**
   ...
   [RegressionConfig](../rust_core/src/bayesian_regression.rs#L20) accepts a general
   ...
   [Forecasting](../rust_core/src/bayesian_regression.rs#L370), however, adds innovations
...
2. **High: Exponential predictive simulation silently changes the distribution.**
   ...
   Both [posterior prediction](../python_bindings/src/lib.rs#L2782) and
   [prior prediction](../python_bindings/src/lib.rs#L3732) clamp the rate and generated
...
3. **Medium: Rust sampler configuration validation is weaker than Python's.**
   ...
   [sample_bound](../rust_core/src/sampler.rs#L212) validates `target_accept`, binding,
...
4. **Medium: duplicate likelihood names overwrite predictive results.**
   ...
   Both likelihood terms enter the model, but [dictionary packaging](../python_bindings/src/lib.rs#L3788)
...
5. **Medium: the forecasting study's score is not standard WIS.**
   ...
   [weighted_interval_score](../demo-docs/run_rustmc_forecast.py#L145) divides by
   ...
   Separately, the GP/Prophet baseline's [wis_95](../demo-docs/baselines/gp_prophet_baselines.py#L110)
...
|---|---|---|
| Unused core dependency | `ndarray` in [rust_core/Cargo.toml](../rust_core/Cargo.toml#L19), with no use in core source, examples, or tests | Remove the core's direct dependency; keep the binding dependency, which is used. Transitive dependencies may still bring ndarray into the build. |
| Unused abstraction | [distributions::Distribution](../rust_core/src/distributions.rs#L3) has no implementations or consumers in the repository | Deprecate/remove it or replace it with a deliberately designed extension interface. It does not currently enable custom distributions. |
| Dead argument | [batch_sample](../rust_core/src/sampler.rs#L448) accepts `(Graph, Vec<f64>)` and explicitly discards `obs_y` at line 483; Python supplies an empty vector | Introduce an internal graph-only path and retire the old argument through a compatibility wrapper. |
| Obsolete helper | [Graph::normal_obs_predictors](../rust_core/src/graph.rs#L615) has no repository callers and still describes a Normal-only surface | Retire in favor of `observation_heads`. |
| Unused convenience helper | [Evaluator::vec_to_owned](../rust_core/src/autodiff.rs#L191) has no repository callers | Retain only if intended public API; otherwise deprecate. |
| Legacy artifact subsystem | [compiled_model.rs](../rust_core/src/compiled_model.rs) is about 1,550 lines; its data-owning JSON format is separate from Python `CompiledModel` | It is tested and publicly exported, so it is not simply dead code. Isolate it as legacy and design one versioned replacement if serialization is a priority. |
| Duplicate evaluator | [forward/grad_logp](../rust_core/src/autodiff.rs#L1076) duplicate much of the production `Evaluator`; gradient checks use them | Preserve the reference value while moving it to a clearly named reference module or feature. Maintain independent numerical checks before retiring anything. |
| Stale product documentation | [README limitations](../README.md#L285) still say forecasting lacks covariates and positive observations; [ROADMAP](../ROADMAP.md#L25) starts with a future 0.9 gate | Update against the shipped 0.11 capabilities so future work is not planned from obsolete gaps. |
...
The Python binding [lib.rs](../python_bindings/src/lib.rs) has 7,387 lines. It includes
...
The current [MuExpr](../python_bindings/src/lib.rs#L431) supports constants, parameters,
...
2. Introduce explicit dimensions and validated indexing. Currently
   [all vector-like data share one length](../python_bindings/src/lib.rs#L1391), and
   [DataBinding](../rust_core/src/data.rs#L65) stores one global `n_obs`. This blocks a
...
[FitResult::posterior_predictive](../python_bindings/src/lib.rs#L2716) only uses the
...
[Generic batches](../rust_core/src/sampler.rs#L348) seed by position, discard successful
...
[forecast executor](../rust_core/src/forecast_batch.rs#L29) already provides stable IDs,
...
misspecification cases. The retained [batch benchmark](../benchmarks/results/2026-09-08-forecast-batch.md)
