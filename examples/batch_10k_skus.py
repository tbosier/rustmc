...
benchmarks/RESULTS_TEMPLATE.md / PR description for the audit):
  - The docstring and README previously said "10,000 SKUs"; N_SKUS below
    ...
    in this repo — do not restate it until one exists. This script scales
    linearly in N_SKUS, so a 10,000-SKU run is a config change away, but
    it has not been run and its numbers must not be assumed.
...
frameworks: many independent posterior inferences in parallel, using a
single Rayon thread pool, with zero Python in the inner loop.
...
with pt.phase("postprocess"):
    ...
    # Aggregate R-hat/ESS across all (SKU x parameter) posteriors using the
    # same rank-normalized split R-hat / bulk ESS estimator as PyMC's side
    # (arviz), computed directly on rustmc's raw (chain, draw) samples.
    import arviz as az
    rhats, esses = [], []
    for r in results:
...
print("(both engines: same chains/warmup/draws/seed per SKU, so ESS/s and "
      "R-hat are directly comparable; MAE above is a point-forecast metric, "
